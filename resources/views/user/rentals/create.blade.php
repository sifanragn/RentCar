...
/* ===== DRIVER SECTION ===== */
.driver-section { margin-top: 25px; background: #fff; border-radius: 14px; padding: 22px 20px; border: 1.5px solid #e0e0e0; box-shadow: 0 3px 10px rgba(0,0,0,0.05);}
...
.driver-dropdown { position: relative; width: 100%; user-select: none;}
.driver-dropdown .selected { display: flex; justify-content: space-between; align-items: center; padding: 10px 12px; border: 1.5px solid #ccc; border-radius: 8px; background: #fafafa; cursor: pointer; }
.driver-dropdown .options { position: absolute; width: 100%; background: #fff; border: 1.5px solid #ddd; border-radius: 8px; margin-top: 6px; box-shadow: 0 4px 10px rgba(0,0,0,0.1); list-style: none; padding: 8px 0; max-height: 250px; overflow-y: auto; display: none; z-index: 99;}
...
.driver-selected button { background: #000; color: #fff; font-weight: 600; padding: 8px 14px; border-radius: 8px; border: none; cursor: pointer;}
.driver-selected button:hover { background: #222; }

/* ===== Modal ===== */
.modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.55); display: none; justify-content: center; align-items: center; z-index: 1000;}
.modal-box { background: #fff; border-radius: 18px; padding: 32px 25px 35px; width: 90%; max-width: 400px; text-align: center; box-shadow: 0 10px 30px rgba(0,0,0,0.18);}
.modal-box img { width: 110px; height: 110px; border-radius: 14px; object-fit: cover; border: 2px solid #000; margin: 10px auto 14px auto;}
.modal-box h3 { margin-bottom: 16px; font-size: 20px; color: #111; font-weight: 700;}
.modal-box p { margin: 6px 0; font-size: 15px; color: #333; background: #fafafa; padding: 6px 10px; border-radius: 6px; text-align: left;}
.modal-box .close-btn { position: absolute; top: 10px; right: 15px; border: none; background: none; font-size: 22px; cursor: pointer; color: #888; }
.modal-box .close-btn:hover { color: #000; }

#popup-menunggu, #popup-alert {
  display: none;
...
#popup-menunggu .popup-content, #popup-alert .popup-box {
  background: #fff; border-radius: 18px; padding: 25px; width: 330px; text-align: center;
}
</style>
@endsection
...
<div class="container-wrapper">
  ...
  <div class="card">
    ...
    <form id="rentalForm" action="{{ route('user.rentals.store', $car->car_id) }}" method="POST">
      ...
      <div id="lokasiRental" style="display:none;">
      ...
      <div id="alamatUser" style="display:none;">
...
<div id="popup-menunggu">
  <div class="popup-content">
    ...
    <a href="{{ route('user.verifikasi.index') }}" style="background:#0d6efd;color:#fff;padding:10px 14px;border-radius:6px;text-decoration:none;">Unggah Sekarang</a>
  </div>
</div>
...
window.addEventListener('load', () => {
  ...
  function hitungTotal() {
    ...
    if (diff < 6) { priceBox.innerHTML = `<strong>❗Minimal sewa 6 jam</strong>`; return; }
    const total = hargaPerJam * diff;
    priceBox.innerHTML = `<strong>Total Estimasi:</strong> Rp${total.toLocaleString('id-ID')}<br><small>(${diff.toFixed(1)} jam × Rp${hargaPerJam.toLocaleString('id-ID')}/jam)</small>`;
  }

  mulai.addEventListener('change', () => { validateJam(mulai); hitungTotal(); });
  selesai.addEventListener('change', () => { validateJam(selesai); hitungTotal(); });

  function validateJam(input){
    const hour = new Date(input.value).getHours();
    if (hour < 8 || hour > 22){
      alert('⚠️ Jam penyewaan hanya antara 08:00–22:00 WIB.');
      input.value='';
    }
  }
  ...
  window.selectDriver = el=>{
    document.querySelector('#driverDropdown .selected span').innerHTML = `<img src="${el.dataset.foto}" style="width:30px;height:30px;border-radius:8px;margin-right:6px;"> ${el.dataset.nama}`;
    document.getElementById('driver_id').value = el.dataset.id;
    document.getElementById('driverSelected').style.display = 'block';
  };
  window.toggleDriverList = sel=>{
    const list=document.getElementById('driverList');
    list.classList.toggle('show', sel.value==='ya');
  };
